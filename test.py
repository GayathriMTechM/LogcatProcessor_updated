...
class SmsMessageProcessor:
    ...
    def read_file(self):
        with open(self.filename, 'r',encoding='utf-8',errors='ignore') as file:
            for line in file:
                if "SmsMessage: mno = " in line or "CS/SmsMessageSent:" in line:
                    self.extract_info(line)
...
class GsmConnectionProcessor:
    def __init__(self, filename):
        ...
        self.patterns = [
            ...
            # (r'Telephony: GsmConnection: refreshCodec: codec changed; old=(?P<oldCodec>\d+), new=(?P<newCodec>\d+)', ['oldCodec', 'newCodec']),
    ...
    def read_file(self):
        with open(self.filename, 'r',encoding='utf-8',errors='ignore') as file:
            for line in file:
                if "Telephony: GsmConnection:" in line:
                    self.extract_info(line)
...
def merge_and_store_data(filename, project_name, project_id, test_case_name, tc_id, test_exec_id, iteration_id, device_type, id, current_iteration_number, mongodb_url, collection_name, db_name):
    ...
    smsmessage_processor= SmsMessageProcessor(filename)
    smsmessage_processor.read_file()
    smsmessage_records = smsmessage_processor.data
    ...
    gsmconnection_processor = GsmConnectionProcessor(filename)
    gsmconnection_processor.read_file()
    gsmconnection_records = gsmconnection_processor.data
    

    for records in [kpi_records, telecom_records, sipmsg_records, smsservicemodule_records, resipsms_records, imssmsdispatcher_records,smsmessage_records, stpservice_records, imsphonecall_records, networkcontroller_records, gsmconnection_records]:
...
if __name__ == "__main__":
    filename = "C:/Users/SS00997234/Desktop/MyProjects/Dish Project/Logcats/Logcat-UX_44_SMS_Tests-SMS-1-REFERENCE-R5CT92MM65A--10302023061034_ExecID-1789989_IterID-3720020.txt" 
    ...
    collection_name = "dish_14_nov_11"
