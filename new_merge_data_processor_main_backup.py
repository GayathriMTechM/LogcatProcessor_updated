    ...
    def extract_info(self, line):
        ...
        for pattern, keys in self.patterns:
            ...
            if match:
                ...
                entry = {"Timestamp": timestamp, "Type": "Telecom", "Call ID": self.event_id, "Event": event, "Method": tag } #removed subevent as per new change in tag breakdown
...
class ImsSmsDispatcherDataProcessor:
    ...
    def __init__(self):
        ...
        self.patterns = [
            ...
            (r'ImsSmsDispatcher: onSendSmsResult: token =(?P<token>\d+) messageRef =(?P<messageRef>\d+) reason =(?P<State>\d+) PhoneId : \[(?P<PhoneId>\d+)\]', ['token', 'messageRef', 'State', 'PhoneId']),
...
class STFServiceDataProcessor:
    ...
    def __init__(self):
        ...
        self.patterns = [
            ...
            (r'STFService: onLocationUpdate: Location\[fused (?P<latitude>.*?),(?P<longitude>.*?) hAcc=(?P<hAcc>.*?) et=(?P<et>.*?) alt=(?P<alt>.*?) vAcc=(?P<vAcc>.*?)\]', ['latitude', 'longitude', 'hAcc', 'et', 'alt','vAcc' ]),
            (r'STFLocationUtils: onLocationResult: Location\[fused (?P<latitude>.*?),(?P<longitude>.*?) hAcc=(?P<hAcc>.*?) et=(?P<et>.*?) alt=(?P<alt>.*?) vAcc=(?P<vAcc>.*?)\]', ['latitude', 'longitude', 'hAcc', 'et', 'alt','vAcc' ])
    ...
    def extract_info(self, line):
        ...
        for pattern, keys in self.patterns:
            ...
            if match:
                ...
                elif any(keyword in line for keyword in ['sendNetworkClass:  CellSignalNr:', 'Location: provider fused;', 'STFService: sendNetworkClass:  CellSignalLte:', 'STFService: sendNetworkClass:  EPDGInfo:']):
                    tag=pattern.split("(")[0]
                    parts = tag.split(': ')
                    event = parts[0]
                    method = parts[1]
...
class ImsPhoneCallProcessor:
    ...
    def __init__(self):
        ...
        self.patterns = [
            ...
            (r'ImsCall : callSessionTerminated :: (?P<header>.+) reasonInfo=ImsReasonInfo :: {(?P<State>.+)} ImsCall=\[(?P<Content>.+)\]', ["State", "Content"]),
            (r'ImsCall : processCallTerminated :: reason=ImsReasonInfo :: {(?P<State>.+)} userInitiated = (?P<Content>.+)', ["State", "Content"]),
                        ...
                        if key in ['fromCode', 'message']:
                            ...
                            if " reasonInfo=ImsReasonInfo :: " in line or "ImsCall : processCallTerminated :: " in line: 
                                ...
                                for key, value in key_value_matches:
                                    ...
                                    if key in ["audioQuality",  "bitrateKbps",  "bandwidthKhz",  "videoQuality", "videoDirection", "rttMode", "hasRttAudioSpeech" ]:
                                        entry['Call Event'] += f"{key}:{value.strip(',')}  "
                                    elif key=='audioDirection':
                                        entry[key] = value.strip(",")
                                    elif key =='networkType':
                                        entry["Network Type"] = value
...
class GsmConnectionProcessor:
    def __init__(self):
        ...
        self.patterns = [
            ...
            (r'Telephony: GsmConnection: refreshCodec: codec changed; old=(?P<oldState>.+), new=(?P<Value>.+)', ['Value']),
            (r'Telephony: GsmConnection: refreshCodec: bitrate changed; old=(?P<oldState>.+), new=(?P<Value>.+)', ['Value']),
            (r'Telephony: GsmConnection: refreshCodec: bandwidth changed; old=(?P<oldState>.+), new=(?P<Value>.+)', ['Value']),
...
class ShannonImsDataProcessor:
    def __init__(self):
        ...
        self.patterns = [
            ...
            (r'SHANNON_IMS: \d+ \[RULE\] (?P<State>.+)\[CellConnectivity VoPS:(?P<VoPS>\d+) EMC:(?P<EMC>\d+) RAT:(?P<RAT>\S+).*?RSRP:(?P<RSRP>[-\d]+) SLID:(?P<SLID>\d+)(?: \([^)]+\))?', ['State', 'RAT', 'VoPS', 'EMC', 'RSRP']),
            (r'SHANNON_IMS: \d+ \[CONN\] RAT: (?P<RAT>\d+), Cell ID: (?P<CellID>\d+), LAC/TAC Info: (?P<LAC_TAC>\d+)(?: \([^)]+\))', ['RAT', 'CellID', 'LAC_TAC']),            
            (r'SHANNON_IMS: \d+ \[HELP\] (?P<State>.+)  \[SLID:(?P<SLID>\d+)  Active:(?P<Active>\w+)]', ['State']),
            (r'SHANNON_IMS: \d+ \[CONN\] Call - (?P<State>.+)\((?P<extra>.*?)\]', ['State']),
            (r'SHANNON_IMS: \d+ \[CONN\] VideoCallStatus. CallId:(?P<CallId>.+) Active:(?P<Active>.+) \[(?P<extra>.*?)\)', ['CallId', 'Active']),
            ...
            ['AudioQuality', 'MinBitrate', 'MaxBitrate', 'CurrentBitrate', 'MinBandwidth', 'MaxBandwidth', 'CurrentBandwidth', 'LimitVoiceBand']),
    
            (r'SHANNON_IMS: \d+ \[ADAP\] Sending hangup for call \[(?P<CallId>\d+)\] with reason \[(?P<State>[^\]]+)\] \[SLID:(?P<SLID>\d+) \]', ['CallId', 'State', 'SLID']),
            (r'SHANNON_IMS: \d+ \[CONN\] -> (?P<State>\S+)  \[Call:(?P<CallStatus>\w+)] \[Current:(?P<CurrentInfo>.+?)\]', ['State']),
            (r'SHANNON_IMS: \d+ \[CALL\] RAT type is set to =>(?P<State>\S+)', ['State']),
            (r'SHANNON_IMS: \d+ \[ADAP\] sendRtpThreshold with callId: (?P<CallId>\d+) rtpJitter: (?P<RtpJitter>\d+) rtpPacketLossRate: (?P<RtpPacketLossRate>\d+) PktLossThrInterval: (?P<PktLossThrInterval>\d+) HandoverNoRtpInterval: (?P<HandoverNoRtpInterval>\d+) RtpInactivityThreshold: (?P<RtpInactivityThreshold>\d+) NotifyCurrentStatus: (?P<NotifyCurrentStatus>\d+) \[SLID:(?P<SLID>\d+) \]', ['CallId', 'RtpJitter', 'RtpPacketLossRate', 'PktLossThrInterval', 'HandoverNoRtpInterval', 'RtpInactivityThreshold', 'NotifyCurrentStatus', 'SLID']),
            (r'SHANNON_IMS: \d+ \[CALL\] (?P<State>.+? \[\] ) ', ['State']),
            (r'SHANNON_IMS: \d+ \[CALL\] (?P<State>[^\[]+)\[com\.shannon\.imsservice\.call\.ImsCall\$Listener@[a-fA-F\d]+\] \(ImsCall\$Listener%callSessionStarted:(?P<extra>\d+)\)', ['State']),
            ...
            ['CallDuration', 'NumRtpTrans', 'NumRtpRecv', 'NumRtpTransLost', 'NumRtpNotRecv', 'AvgRelJitter', 'MaxRelJitter', 'AvgRoundTripTime', 'RtpInactivityDetected', 'RxSilenceDetected', 'TxSilenceDetected', 'NumOfVoiceFrames', 'NumOfNoDataFrames', 'NumOfDroppedRtpPackets', 'MinPlayoutDelay', 'MaxPlayoutDelay', 'NumOfSidPackets', 'NumOfDupPackets']),
            ...
            (r'SHANNON_IMS: \d+ \[CALL\] Sending RTP Threshold with loss-rate: (?P<lossRate>\d+) jitter: (?P<jitter>\d+) inactivity: (?P<inactivity>\d+) isBecauseOfHandover: (?P<isBecauseOfHandover>\S+) (?P<Content>.+)', ["lossRate","jitter", "inactivity", "isBecauseOfHandover", "Content"]),
            ...
            (r'SHANNON_IMS: \d+ \[CALL\] currentRatType:(?P<State>.+)ratTypeDuringDialing:(?P<extra>.+){(?P<Content>.+)}', ["State" , "Content"]),
            (r'SHANNON_IMS: \d+ \[NETW\] #:PURP:# UNSOL {RILC_UNSOL_PHYSICAL_CHANNEL_CONFIGS} :   band : (?P<bands>\d+) (?P<extra>.*?)', ['bands']),
            (r'SHANNON_IMS: \d+ \[NETW\] #:PURP:# UNSOL {RILC_UNSOL_AUDIO_RINGBACK} : ringback-state : (?P<ringbackState>\d+) (?P<extra>.*?)', ['ringbackState']),
            (r'SHANNON_IMS: \d+ \[NETW\] #:PURP:# UNSOL {RILC_UNSOL_WB_AMR_REPORT} : status : (?P<State>\d+) (?P<extra>.*?)', ['State']),
                ...
                for key in keys:
                        ...
                        if value is not None:
                            if '#:PURP:# UNSOL {RILC_UNSOL_AIMS_CODEC_INFO}' in line:
                                ...
                                key_value_matches = re.findall(key_value_pattern, value)
                                if ' listening ' in line or 'currentRatType:' in line:
                                    # print(key_value_matches)
                                    entry["Call Event"]=''
                                    for key, value in key_value_matches:
                                        if key in ["audioQuality",  "bitrateKbps",  "bandwidthKhz",  "videoQuality", "videoDirection", "rttMode", "hasRttAudioSpeech", "WFC"]:
                                            entry['Call Event'] += f"{key}:{value.strip(',')}  "
                                        elif key=='audioDirection':
                                            entry[key] = value.strip(",")
                                    ...
                                    for key, value in key_value_matches:
                                        # print(key_value_matches)
                                        if key in ["call-type", "call-state" , "multiparty", "state-reason" ,"end-cause",  "audio-quality", "sip-error-code",  "sip-reason-header-num",   "reason-header : end-cause", "sip-error-code" , "sip-cause-code" ,  "sip-reason-text" ,   "q850-cause-code" , "q850-reason-text" ,  "current-bitrate",   "min-bitrate", "max-bitrate", "current-bandwidth",  "min-bandwidth", "max-bandwidth" ]:
                                            entry['Call Event'] += f"{key}:{value.strip(',')}  "
                                        else:
                                            entry[key] = ''
                                    ...
                                    for key, value in key_value_matches:
                                        if key in ["audioQuality",  "bitrateKbps",  "bandwidthKhz",  "videoQuality", "videoDirection", "rttMode", "hasRttAudioSpeech" ]:
                                            entry['Call Event'] += f"{key}:{value.strip(',')}  "
                                        elif key in ['EPSFB', '5GHO', 'Redirect_Not_triggered', 'epsfb_lte_ho', 'vonr_ue_but_call_on_lte_nr_ho_redirect', 'vonr_ue_but_call_on_lte_nr_ho_cmd']:
                                            # entry['Call Event'] += f"{key}:{value.strip(',')}  "
...
class RILJDataProcessor:
    def __init__(self):
        ...
        self.patterns = [
            ...
            r'mCellConnectionStatus=(?P<mCellConnectionStatus>.+)\sCellIdentityLte:{\smCi=(?P<ci>.+)\smPci=(?P<pci>.+)\smTac=(?P<tac>.+)\smEarfcn=(?P<earfcn>.+)\smBands=\[(?P<bands>.+)\]'
            r'\smBandwidth=(?P<bandwidth>.+)\smMcc=(.*?)\smMnc=(.*?)\smAlphaLong=(?P<alphaLong>.+)\smAlphaShort=(.*?)mAdditionalPlmns=(.*?)\smCsgInfo=(.*?)CellSignalStrengthLte:\srssi=\d+\srsrp=(?P<rsrp>.+)\srsrq=(?P<rsrq>.+)\srssnr=(?P<sinr>.+)qiTableIndex=\d+\scqi=\d+\sta=(.*?)\slevel=(.*?)\sparametersUseForLevel=\d+\sandroid\.telephony\.CellConfigLte[^{]+{[^}]+isEndcAvailable\s*=\s*(?P<EndcAvailable>.+)\s*}\s*}\]\s*\[(?P<subsID>.+)]',
            ['networkType', 'mRegistered', 'mCellConnectionStatus', 'ci', 'pci', 'tac', 'earfcn', 'bands', 'bandwidth', 'alphaLong', 'rsrp', 'rsrq', 'sinr', 'EndcAvailable', 'subsID']),

            (r'RILJ\s*:\s*\[UNSL\]<\sUNSOL_PHYSICAL_CHANNEL_CONFIG\s\[{mConnectionStatus=(?P<mCellConnectionStatus>.+),mCellBandwidthDownlinkKhz=(?P<dlbw>.+),mCellBandwidthUplinkKhz=(?P<ulbw>.+),mNetworkType=(?P<networkType>.+),mFrequencyRange=(.*?),mDownlinkChannelNumber=(?P<dlchannel>.+),mUplinkChannelNumber=(?P<ulchannel>.+),mContextIds=(.*?)\],mPhysicalCellId=(?P<pci>.+),mBand=(?P<bands>.+),mDownlinkFrequency=(?P<dlfreq>.+),mUplinkFrequency=(?P<ulfreq>.+)}]\s*\[(?P<subsID>.+)]', ['mCellConnectionStatus','dlbw', 'ulbw', 'networkType', 'dlchannel', 'ulchannel', 'pci', 'bands', 'dlfreq', 'ulfreq', 'subsID'])
        ...
        for pattern, keys in self.patterns:
            ...
            if match:
                ...
                if 'GET_CELL_INFO_LIST' in line:
                    ...
                    tag_match = re.search(r'RILJ\s*:\s*\[UNSL\]<\sUNSOL_PHYSICAL_CHANNEL_CONFIG\s\[{mConnectionStatus=.*?', line)
...
class NetworkTypeControllerProcessor:
    def __init__(self):
        ...
        self.patterns = [
            ...
            (r'NetworkTypeController:\s*\[\d+\](?:.+)\[{mConnectionStatus=(?P<mCellConnectionStatus>.+),mCellBandwidthDownlinkKhz=(?P<dlbw>.+),mCellBandwidthUplinkKhz=(?P<ulbw>.+),mNetworkType=(?P<networkType>.+),mFrequencyRange=(.*?),mDownlinkChannelNumber=(?P<dlchannel>.+),mUplinkChannelNumber=(?P<ulchannel>.+),mContextIds=(.*?)\],mPhysicalCellId=(?P<mPhysicalCellId>.+),mBand=(?P<bands>.+),mDownlinkFrequency=(?P<dlfreq>.+),mUplinkFrequency=(?P<ulfreq>.+)}]', ['mCellConnectionStatus', 'dlbw', 'ulbw', 'networkType', 'dlchannel', 'ulchannel', 'mPhysicalCellId','bands','dlfreq','ulfreq'])
        ]
    def extract_info(self, line):
        timestamp_match = re.search(r'^(\d+-\d+ \d+:\d+:\d+\.\d+)', line)
        #print(line)
        for pattern, keys in self.patterns:
            ...
            if match:
                ...
                if "Entering" in line:
                    ...
                    tag_match = re.search(r'NetworkTypeController:\s*\[\d+\]\s*Physical\schannel\sconfigs\supdated:\s*', line)
                    if tag_match:
                        tag = tag_match.group(0)
                        parts = tag.split(' ')
                        event = method = parts[0].strip(':')
...
class GeneralProcessor:
    def __init__(self,tcList= None,info=None, mongodb_url= None, db_name= None, collection_name= None):
        ...
        self.common_keys = [
            "Timestamp", "Type", "Event", "Method", "Sub-Event", "Value", "State", "cause", "Network Type",
            "audioDirection", "Call Event", "number", "result", "HPLMN", "sim_state", "Serving PLMN", "Roaming", "NW sel mode", "Service",
            "MM5G", "NR5G_RSRP", "NR5G_SINR", "NR5G_RSRQ", "NR_RRC", "NR_SSB_Index", "Carrier",
            "NR_ARFCN", "NR_PCI", "NR_BAND", "NR_DL Scheduling", "NR_BLER", "NR_BW",
            "NR_SA_RLF Count", "NR_Tx Phy Channel Type", "NR_Tx Pwr", "NR_CDRX", "NETWORK", "FINAL IMEI Certi",
            "Call ID", "SIP_v", "CSEQ", "handle", "contentType",
            "domain", "format", "token", "PhoneId", "messageRef", "message_class",
            "mno", "sendResult", "errorClass", "errorCode",  "rssi"
            "Alpha Tag", "asulevel", "csiCqiTableIndex", "parametersUseForLevel", "csiCqiReport",
            "rsrp", "csiRsrp", "mAdditionalPlmns", "csiRsrq", "sinr", "nci", "rsrq",
            "tac", "latitude", "longitude", "network", "override",  "setCallRadioTech", "audioQuality",
            "hAcc", "et", "alt", "vAcc", "QCI", "token", "bytes", "Call Status", "NormalECCCallID",
            "RAT", "VoPS", "EMC", "RSRP", "SLID", "Header", 'CellID', 'LAC_TAC',
            "CallStateValue", "ForegroundCallState", "BackgroundCallState", "DisconnectCause", "PreciseDisconnectCause",
            "SLID", "Active", 'AudioQuality', 'MinBitrate', 'MaxBitrate', 'CurrentBitrate', 'MinBandwidth', 'MaxBandwidth', 'CurrentBandwidth', 'LimitVoiceBand', "SLID",
            "CallStatus", "CurrentInfo", "RtpJitter", "RtpPacketLossRate", "PktLossThrInterval",
            "HandoverNoRtpInterval", "RtpInactivityThreshold", "NotifyCurrentStatus","JitterThreshold",
            "PktLossThreshold",  "HandleMessage", "Registered", "CellConnectionStatus",
            "ci", "pci", "earfcn", "bands", "bandwidth", "alphaLong",  "rssnr", "ta", "EndcAvailable", "level",
            'CallDuration', 'NumRtpTrans', 'NumRtpRecv', 'NumRtpTransLost', 'NumRtpNotRecv', 'AvgRelJitter', 'MaxRelJitter', 'AvgRoundTripTime', 
            'RtpInactivityDetected', 'RxSilenceDetected', 'TxSilenceDetected', 'NumOfVoiceFrames', 'NumOfNoDataFrames', 'NumOfDroppedRtpPackets', 'MinPlayoutDelay', 'MaxPlayoutDelay', 'NumOfSidPackets', 'NumOfDupPackets',"Content"
            "ReasonCode", "SLID", "CID", "CURI", "MLT", "serviceType", "callType", "restrictCause", "audioQuality", "audioDirection", 
            ...
            "emergencyUrns", "emergencyCallRouting", "emergencyCallTesting", "hasKnownUserIntentEmergency", "mRestrictCause", "mCallerNumberVerstat", "mAcceptedRtpHeaderExtensions", "CST", "HRS", "IC", "ECC", "WFC", "VCC", "OIR", "CRS", 
            "CUI", "CCID", "MP", "CCS", "L", "PCR", "LRS", "lossRate","jitter", "inactivity", "isBecauseOfHandover",
            "ringbackState", 'mCellConnectionStatus', 'DL BW', 'UL BW', 'DL Channel', 'UL Channel', 'mPhysicalCellId','DL Freq','UL Freq', "Subs ID",
            "dns_server_4", "ip_address_1", "dns_server_3", "ip_address_2", "dns_server_2", "dns_server_1", "max_downstream_bandwidth", "is_secure", "tunneling_protocol", "max_upstream_bandwidth",
            "speed", "meters_on_segment", "toast", "bsic", "ber", "lac", "mTa", "cid"
    ...
    def aggregate_output_data(self, filename=None):
        ...
        for records in [self.modemservice_processor.group_data(), self.telecom_processor.data, self.sipmsg_processor.data, 
                        self.sms_service_processor.data, self.resip_sms_processor.data, self.ims_smsdispatcher_processor.data, 
                        self.sms_message_processor.data, self.stf_service_processor.data,self.ims_phonecall_processor.data,self.gsm_processor.data, 
                        self.network_processor.data, self.ril_processor.data, self.audio_processor.data, self.shannon_ims_processor.data,
                        self.rilj_data_processor.data, self.voiceCall_persistAtoms_processor.data, self.calldata_collector_prcoessor.data, 
    ...
    def merge_and_store_data(self, filename, project_name, project_id, test_case_name, tc_id, test_exec_id, iteration_id, device_type, id, current_iteration_number, mongodb_url, collection_name, db_name):
        ...
        for record in self.combined_data:
            # record = {key: value for key, value in record.items() if key in self.common_keys}
            record = {key: value for key, value in record.items() if key in self.common_keys and value not in [None, 'null', '', '0', ' ']}
...
if __name__ == "__main__":
    ...
    filename = "C:/Users/GM001010686/Desktop/Logcat-Processor-main/Logcats/test.txt"
    project_name = 1
    project_id = 2
    test_case_name = 3
    tc_id = 4
